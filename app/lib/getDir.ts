...
export const getDir = async (id: ID): Promise<Dir> => {
  const dir = db.prepare("SELECT * FROM directory WHERE id = ?").get(id) as DirModel | undefined; // 데이터 가져오기
  if (!dir) {
    throw new Error("Directory not found");
  }

  let children = db.prepare("SELECT * FROM directory WHERE parent = ?").all(id) as DirModel[];
